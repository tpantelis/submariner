...
var _ = Describe("Leader election tests", func() {
	...
	When("renewal of the leader lease fails", func() {
		It("should re-acquire the leader lease after the failure is cleared", func() {
			...
			updateLeaseUpdatePermission(cluster, gateway.Namespace, true)

			DeferCleanup(func() {
				updateLeaseUpdatePermission(cluster, gateway.Namespace, false)
			})
...
func updateLeaseUpdatePermission(cluster framework.ClusterIndex, ns string, remove bool) {
		...
		func(role *rbacv1.Role) (*rbacv1.Role, error) {
			for i := range role.Rules {
				if role.Rules[i].APIGroups[0] == coordinationv1.GroupName && role.Rules[i].Resources[0] == "leases" {
					if remove {
						role.Rules[i].Verbs, _ = slices.Remove(role.Rules[i].Verbs, "update", func(e string) string {
							return e
						})
					} else {
						role.Rules[i].Verbs, _ = slices.AppendIfNotPresent(role.Rules[i].Verbs, "update", func(e string) string {
							return e
						})
					}
